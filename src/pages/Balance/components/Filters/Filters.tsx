...
const getFilters = (tableName: string) => ({
  FilterSelect: hocFilter(Select, { name: 'leaveStatus', tableName }),
  FilterDeptSelect: hocFilter(DeptSelect, { name: 'deptIds', tableName }),
});

const Filters: FC<{ tableName: string }> = ({ tableName }) => {
  const { FilterSelect, FilterDeptSelect } = useMemo(() => getFilters(tableName), [tableName]);
  return (
    <div className="pg-balance--filters">
      ...
      />
      <FilterDeptSelect className="pg-balance--filters--dept" showCompany hasPermission />
      <FilterSelect
