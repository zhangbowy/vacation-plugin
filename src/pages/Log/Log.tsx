...
const columns = [
  {
    ...
    render: (createTime: number) => moment(createTime).format('YYYY-MM-DD HH:mm'),
    width: '26.942%',
  },
  {
    title: '操作人员',
    ...
    render: ({
      ...
      if (operateFileUrl) {
        ...
        return (
          <>
            <span>{`${title}：`}</span>
            <a href={operateFileUrl} download={name} target="_blacnk">
              {name}
            </a>
          </>
        );
      ...
      return (
        <>
          <p>{title}</p>
          {changeDatas.map(({ field, before, after }, i) => {
            const key = i;
            return (
              <p key={key}>{`${field}：${
                operateType != 1 ? `修改前${before},` : ''
              }修改后${after}`}</p>
            );
          })}
        </>
