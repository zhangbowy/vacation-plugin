...
const getInitialState = (payload = {}) => {
  ...
  if (type !== 'user' && showCompany) {
    r.paths = [];
    r.options.departments = [{ name: topName, id: '1' }];
    return r;
  }
      ...
      if (action.itemType === 'user') {
        ...
        return {
          ...
          value: {
            ...
            departments: [...departments.slice(0, index), ...departments.slice(index + 1)],
          },
        };
      }
    case 'batch add':
      return {
        ...state,
        value: {
          users: [...state.value.users, ...action.users],
          departments: [...state.value.departments, ...action.departments],
        },
      };
    case 'update value':
      return {
    ...
    case 'add item':
      if (action.itemType === 'user') {
        return {
          ...state,
          value: {
            users: [...state.value.users, action.item],
            departments: state.value.departments,
          },
        };
      } else {
        return {
          ...
          value: {
            users: state.value.users,
            departments: [...state.value.departments, action.item],
          },
        };
      }
    default:
